...
def exponential_decay_list(init_weight, decay_rate, num_steps):
    ...
    return torch.tensor(weights).cuda()  # 移到 CUDA
...
class MarigoldDepthInpaintingPipeline(DiffusionPipeline):
    """
    ...
    def __call__(
        ...
        with torch.no_grad():
            ...
            mask = torch.nn.functional.interpolate(
                latent_mask, size=(height // self.vae_scale_factor, width // self.vae_scale_factor)
            )
        ...
        with torch.enable_grad():
            ...
            for i in self.progress_bar(
                ...
                for i, t in enumerate(self.progress_bar(self.scheduler.timesteps, leave=False, desc="Diffusion steps...")):
                    if i>=0:
                        for k in self.progress_bar(range(8), desc="Latent Optimizing..."):
                            batch_latent = torch.cat([batch_image_latent, batch_pred_latent], dim=1)  # [B,8,h,w]
                            ori_batch_pred_latent =  batch_pred_latent.clone().detach()

                            batch_pred_latent.requires_grad_(True)
                            batch_pred_latent.retain_grad()
                            batch_image_latent.requires_grad_(True)
                            batch_image_latent.retain_grad()
                            batch_latent.requires_grad_(True)
                            batch_latent.retain_grad()
                            torch.set_grad_enabled(True)
                            optimizer = bnb.optim.AdamW8bit([batch_pred_latent], lr=0.001, weight_decay=0.0)
                            ...
                            for iter, timestep in enumerate(self.progress_bar(self.scheduler.timesteps[i+1:], desc="Iteratvely denoise...")):                             
                                batch_latent = torch.cat([batch_image_latent, batch_pred_latent], dim=1)
                                noise = self.unet(batch_latent, timestep, encoder_hidden_states=text, return_dict=False)[0]  # [B,4,h,w]
                                batch_pred_latent = self.scheduler.step(
                                    noise, timestep, batch_pred_latent, generator=generator
                                ).prev_sample  # [B,4,h,w]
                            '''

                            prediction = self.decode_prediction(batch_pred_latent[0:1])
                            ...
                            #vis = self.image_processor.visualize_depth(prediction.detach())
                            #vis[0].save(f"einstein_inpaint_depth_{i}.png")
                            # breakpoint()
                            
                            # Solution1. Depth MSE Loss
                            # loss = (torch.nn.functional.mse_loss(prediction*mask, gt_depth*mask)) * 1000
                            # Solution2. Disparity MSE Loss
                            # clamped_prediction = prediction.clamp(min=1e-4)[0]
                            # clamped_gt_depth = gt_depth.clamp(min=1e-4)

                            # prediction_disparity = 1 / clamped_prediction[mask == 1]
                            # gt_disparity = 1 / clamped_gt_depth[mask == 1]

                            # loss = torch.nn.functional.mse_loss(prediction_disparity, gt_disparity)
                            # Solution3. Depth Huber Loss
                            # loss = torch.nn.functional.smooth_l1_loss(prediction*mask, gt_depth*mask) * 1000
                            
                            loss = torch.nn.functional.huber_loss(prediction*mask, gt_depth*mask, "mean", delta=0.3) * 1000
                            # Solution4. Depth Huber Loss + Only Get Loss from the near non Masked Area
                            ...
                            unmask = 1 - mask
                            unmask_dilated = dilate_mask(unmask, iterations=5, kernel_size=3)
                            
                            y_indices, x_indices = torch.where(unmask_dilated > 0.5)
                            y_min, y_max = y_indices.min().item(), y_indices.max().item()
                            x_min, x_max = x_indices.min().item(), x_indices.max().item()
                            cropped_prediction = prediction[0, 0, y_min:y_max, x_min:x_max]
                            cropped_gt_depth = gt_depth[0, y_min:y_max, x_min:x_max]
                            cropped_mask = mask[0, y_min:y_max, x_min:x_max]
                            loss = torch.nn.functional.huber_loss(cropped_prediction*cropped_mask, cropped_gt_depth*cropped_mask, "mean", delta=0.3) * 1000
                            ...
                            batch_pred_latent = ori_batch_pred_latent - sync_scheduler[i]*torch.norm(ori_batch_pred_latent) * batch_pred_latent.grad 
                    ...
                    with torch.no_grad():
                        ...
                        if i>=30:
                            for m in range(1): #8
                                ...
                                eps = randn_tensor(batch_pred_latent.shape, generator=generator, dtype=batch_pred_latent.dtype).to(device)
                                batch_pred_latent = torch.sqrt(1- current_beta_t) * batch_pred_latent + torch.sqrt(current_beta_t) * eps
    ...
    def decode_prediction(self, pred_latent: torch.Tensor) -> torch.Tensor:
        ...
        prediction = torch.mean(prediction, dim=1, keepdim=True)  # [B,1,H,W]
